// PAKEExp.cpp: Functions for export to PAK's DAT file.
//
//////////////////////////////////////////////////////////////////////
...
UINT CModelData::ExportPAK_E(CStdioFile* file,CPakExpOpt *peo)
{
...
//Card /6/
	...
	ExportPAKE_Nodes(file);
//	ExportPAKE_Elements(file,&uNGELEM,peo);
// Na silu stampa sve elemente u jednu grupu
	ExportPAKE_Elements_Without_Groups(file,&uNGELEM,peo);

// Card /9/
	ExportPAKE_MatModels(file);
/*
...
//Card /10/
	if(m_LoadArray[0].m_NodalTemps.GetSize()>0) ExportPAKE_TimeFunctions(file);
//Card /11/
	ExportPAKE_PrescTemp(file);

//Card /12/
	ExportPAKE_Loads(file);
/*
//Card /12-6/
...
UINT CModelData::ExportPAKE_Elements(CStdioFile* file,UINT *uNGELEM,CPakExpOpt *peo)
{
	CString str;
	UINT i,j,k,z;
	HProperties pr;
...
//**************************************************************************
UINT CModelData::ExportPAKE_Elements_Without_Groups(CStdioFile* file,UINT *uNGELEM,CPakExpOpt *peo)
{
	CString str;
	UINT i,j,k,z;
	HProperties pr;
...
// Na silu stampa zaglavlje samo za prvu grupu
		...
		//		 str.Format("%5u%5u%5u%5u\n",PAKE_ISO_3D,uNE_3D,INDAX,IZIP);
		//	Na silu stampa da je broj elemenata u grupi jednak ukupnom broju elemenata
				 str.Format("%5u%5u%5u%5u\n",PAKE_ISO_3D,m_ElArray.GetSize(),INDAX,IZIP);
				 file->WriteString(str);
...
UINT CModelData::ExportPAKE_Nodes(CStdioFile* file)
{
...
UINT CModelData::ExportPAKE_MatModels(CStdioFile* file)
{
...
UINT CModelData::ExportPAKE_TimeFunctions(CStdioFile* file)
{
	CString str;
	CPAKString pstr;
	UINT i,j,uMAXTFT=0;
...
UINT CModelData::ExportPAKE_PrescTemp(CStdioFile* file)
{
	HLoads lo;
	UINT i;
	CString str;
...
/*
UINT CModelData::ExportPAKE_Loads(CStdioFile* file)
{
	CString str;
	UINT i,j,uMAXTFT=0;
	HLoads lo;
