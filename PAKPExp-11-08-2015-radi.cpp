...
//Card /7/
	ExportPAKP_Nodes(file);

//Card /8/
//	ExportPAKT_Elements(file,&uNGELEM,peo);
// Na silu stampa sve elemente u jednu grupu
	ExportPAKP_Elements_Without_Groups(file,&uNGELEM,peo);

// Card /9/
	ExportPAKP_PrescTemp(file);
...
UINT CModelData::ExportPAKP_Nodes(CStdioFile* file)
{
...
//**************************************************************************
UINT CModelData::ExportPAKP_Elements_Without_Groups(CStdioFile* file,UINT *uNGELEM,CPakExpOpt *peo)
{
	CString str;
	UINT i,j,k;
	HProperties pr;
...
// ****************************** 1D Elementi *********************************
//****************************************************************************
		case FET_ROD: case FET_BEAM:
			{
				
				uINDAX=0;

				 UINT node_order_quad[20]={0,1,2,3,4,5,6,7,8,8,8,8,8,8,8,8,8,8,8,8};
				 UINT node_order_tri[20]={0,1,2,2,4,5,2,6,8,8,8,8,8,8,8,8,8,8,8,8};
				 UINT *node_order;
...
UINT CModelData::ExportPAKP_PrescTemp(CStdioFile* file)
{
	HLoads lo;
	UINT i;
	CString str;
