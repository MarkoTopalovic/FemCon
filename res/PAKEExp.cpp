...
//Card /6/
	...
	ExportPAKT_Nodes(file);
//	ExportPAKT_Elements(file,&uNGELEM,peo);
// Na silu stampa sve elemente u jednu grupu
	ExportPAKT_Elements_Without_Groups(file,&uNGELEM,peo);

// Card /9/
	ExportPAKT_MatModels(file);
/*
...
//Card /10/
	if(m_LoadArray[0].m_NodalTemps.GetSize()>0) ExportPAKT_TimeFunctions(file);
//Card /11/
	ExportPAKT_PrescTemp(file);

//Card /12/
	ExportPAKT_Loads(file);
/*
//Card /12-6/
